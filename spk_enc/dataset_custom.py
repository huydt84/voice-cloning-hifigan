...
class CustomCodeDataset(torch.utils.data.IterableDataset):
    def __init__(self, training_files, segment_size, code_hop_size, n_fft, num_mels,
        ...
        self.pad_unit = pad
        self.pad = pad * 320 if pad else None
    ...
    def __iter__(self):
        for data in self.data:
            ...
            if self.pad_unit:
                if len(unit) > self.pad_unit:
                    unit = unit[:self.pad_unit]
                else:
                    padding_unit = self.pad_unit - len(unit)
                    unit = np.pad(unit, (0, padding_unit), "constant", constant_values=0)
            ...
            if self._cache_ref_count == 0:
                ...
                if self.pad:
                    if audio.shape[-1] > self.pad:
                        audio = audio[:self.pad]
                    else:
                        padding = self.pad - audio.shape[-1]
