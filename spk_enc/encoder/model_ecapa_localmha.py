...
class SpeakerEncoder(nn.Module):
    def __init__(self, device, loss_device, in_channels=mel_n_channels, channels=512, embd_dim=model_embedding_size):
        super().__init__()
        ...
        self.conv = nn.Conv1d(cat_channels, cat_channels, kernel_size=1).to(device)
        self.pooling = AttentiveStatsPool(cat_channels, 128).to(device)
        self.bn1 = nn.BatchNorm1d(cat_channels * 2).to(device)
        self.linear = nn.Linear(cat_channels * 2, embd_dim).to(device)
        self.bn2 = nn.BatchNorm1d(embd_dim).to(device)

        self.mha = nn.ModuleList([LocalMHA(dim = cat_channels, heads = 8, window_size = 8, dropout=0.1) for _ in range(6)]).to(device)
    ...
    def forward(self, x):
        ...
        for i in range(6):
            out = self.mha[i](out, mask=None) + out
        out = out.transpose(1, 2)
        ...
        # L2-normalize it
        embeds = out / (torch.norm(out, dim=1, keepdim=True) + 1e-5)
